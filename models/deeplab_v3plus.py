...
class ResNet(nn.Module):
    ...
    def _load_pretrained_model(self):
        pretrain_dict = model_zoo.load_url('https://download.pytorch.org/models/resnet50-0676ba61.pth', model_dir='./checkpoints')
        model_dict = {}
...
class Deeplab_V3plus(nn.Module):
    def __init__(self, backbone='resnet50', num_classes=6):
        ...
        self.backbone = ResNet50(output_stride, nn.BatchNorm2d)
        self.aspp = ASPP(output_stride, nn.BatchNorm2d)
        self.decoder = Decoder(num_classes, nn.BatchNorm2d)
    ...
    def get_predicts(self, x, low_level_feat, size=None):
        x = self.decoder(x, low_level_feat)
        if size:
            x = F.interpolate(x, size=size, mode='bilinear', align_corners=False)
        return torch.sigmoid(x)       
